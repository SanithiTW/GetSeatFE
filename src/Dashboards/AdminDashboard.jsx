...
const BookingsView = () => {
    ...
    const loadBookings = async () => {
        ...
        try {
            // 1. Load bookings
            const bookingsSnap = await getDocs(collection(databaseb, "bookings"));
            const bookingsData = bookingsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));

            // 2. Load passengers
            const passengersSnap = await getDocs(collection(databaseb, "passengers"));
            const passengersMap = {};
            passengersSnap.docs.forEach(doc => {
                const data = doc.data();
                passengersMap[doc.id] = data;
            });

            // 3. Load schedules
            const schedulesSnap = await getDocs(collection(databaseb, "schedules"));
            const schedulesMap = {};
            ...
            // 4. Load buses
            const busesSnap = await getDocs(collection(databaseb, "buses"));
            const busesMap = {};
            busesSnap.docs.forEach(doc => {
                const data = doc.data();
                busesMap[data.busNumber] = data; // Use busNumber as key
            });

            // 5. Merge data
            const enrichedBookings = bookingsData.map(b => {
                const passenger = passengersMap[b.userId] || {};
                const schedule = schedulesMap[b.scheduleId] || {};
                const bus = busesMap[schedule.busNumber] || {};

                return {
                    ...b,
                    passengerName: passenger.fullName || 'Unknown Passenger',
                    route: `${schedule.departure || 'N/A'} → ${schedule.arrival || 'N/A'}`,
                    bus: `${bus.busName || schedule.busModel || 'N/A'} (${schedule.busNumber || ''})`,
                    seatNumbers: b.seats?.join(', ') || ''
                };
            });
